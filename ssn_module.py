...
def get_response_files(dir_resp, station_name, t_start):
    ...
    elif station_name.strip() == 'HLIG':
        if t_start >= UTCDateTime(2009, 12, 8) and t_start < UTCDateTime(2018, 8, 13):
            RESP_FILE = os.path.join(
                dir_resp, 'HLIG_IG_20091208_20180813.RESP')
        elif t_start >= UTCDateTime(2018, 8, 13):
            RESP_FILE = os.path.join(
                dir_resp, 'HLIG_IG_20180813_21001231.RESP')
        else:
            print('ERROR: No RESP file for ',
                  station_name, ' at time: ', t_start)
            exit()
